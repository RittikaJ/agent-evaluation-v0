...
st.markdown(
    ...
    "with failure diagnosis, tool efficacy, and cost tracking."
)
...
# Claude Sonnet pricing per million tokens (claude-sonnet-4-6)
_PRICE_PER_M = {
    "input":                  3.00,   # input_tokens (non-cached)
    "output":                15.00,   # output_tokens
    "cache_read":             0.30,   # cache_read_input_tokens
    "cache_creation":         3.75,   # cache_creation_input_tokens (5-min TTL)
}


def estimate_cost_usd(result: dict) -> float:
    """Compute API cost in USD from granular token usage breakdown.

    Falls back to a blended estimate when token_usage_details is unavailable.
    """
    details = result.get("token_usage_details")
    if details:
        ...
        # Non-cached input = total input minus cache hits and cache writes
        non_cached_input = max(0, input_tokens - cache_read - cache_creation)
        return (
            (non_cached_input / 1_000_000) * _PRICE_PER_M["input"]
            + (output_tokens / 1_000_000) * _PRICE_PER_M["output"]
            + (cache_read / 1_000_000) * _PRICE_PER_M["cache_read"]
            + (cache_creation / 1_000_000) * _PRICE_PER_M["cache_creation"]
        )
    # Fallback: blended avg ($3 input / $15 output, ~70/30 mix ≈ $6.60/M)
    return (result.get("token_usage", 0) / 1_000_000) * 6.60
...
default_idx = st.session_state.get("comp_selected_idx", 0)
...
def display_results(result, item, all_scores, judge_scores, plan_scores,
    ...
    """Display the full results for a completed agent run with all 9 metrics."""
    ...
    st.divider()
    st.header("📊 All 9 Metrics at a Glance")

    # Row 1: Deterministic metrics
    ...
    # Row 2: LLM Judge Rubric metrics
    st.subheader("LLM-Judge Rubric Metrics")
    c1, c2, c3 = st.columns(3)
    with c1:
        with st.container(border=True):
            st.metric("🛡️ Groundedness", f"{judge_scores['groundedness']}/5")
    ...
    # Row 3: Plan-Level metrics
    st.subheader("Plan-Level Metrics")
    c1, c2 = st.columns(2)
    with c1:
        with st.container(border=True):
            st.metric("🗺️ Plan Quality", f"{plan_scores['plan_quality']}/5")
    ...
    with c2:
        with st.container(border=True):
            ...
            st.caption("Did the agent follow through on its search plan?")
    ...
    with st.container(border=True):
        ...
        with c5:
            ...
            st.caption(f"{result['token_usage']:,} tokens · {latency_secs:.1f}s")
    ...
    # ══════════════════════════════════════════════════════════════════════
    # Multi-Layer Evaluation View
    # ══════════════════════════════════════════════════════════════════════

    st.header("📊 Multi-Layer Evaluation View")
    st.caption(
        "A comprehensive view of both **turn-level** (per step) and "
        "**end-to-end** (overall task) performance across all 9 metrics."
    )
    ...
    with tab_e2e:
        st.caption("Overall task completion across all 9 evaluation metrics.")
        col1, col2, col3 = st.columns(3)
        with col1:
        ...
        # Visual summary bar — all 9 metrics
        st.markdown("---")
        st.markdown("**All 9 metrics breakdown:**")
        cols = st.columns([2, 1])
        with cols[0]:
            bar_data = {
    ...
    with col4:
        ...
        st.caption("Granular per-tier pricing")
    ...
    if details:
        ...
        input_tok = details.get("input_tokens", 0)
        output_tok = details.get("output_tokens", 0)
        cache_read = details.get("cache_read_input_tokens", 0)
        cache_create = details.get("cache_creation_input_tokens", 0)
        non_cached = max(0, input_tok - cache_read - cache_create)
        with t1:
            st.metric("Input (non-cached)", f"{non_cached:,}")
            st.caption("@ $3.00/M")
        with t2:
            st.metric("Output", f"{output_tok:,}")
            st.caption("@ $15.00/M")
        with t3:
            st.metric("Cache Read", f"{cache_read:,}")
            st.caption("@ $0.30/M")
        with t4:
            st.metric("Cache Creation", f"{cache_create:,}")
            st.caption("@ $3.75/M")
...
if st.button("▶️ Run Agent & Comprehensive Evaluate", type="primary", use_container_width=True):
    ...
    with st.status("Running agent...", expanded=True) as status:
        ...
        experiment_name = f"comp_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        status.update(
            label="✅ Done — comprehensive evaluation scored and uploaded to Langfuse",
            state="complete",
        )

    # Store in session state with comp_ prefix
    st.session_state.comp_agent_result = result
    st.session_state.comp_trace_id = trace_id
    st.session_state.comp_selected_idx = idx
    st.session_state.comp_all_scores = all_scores
    st.session_state.comp_judge_scores = judge_scores
    st.session_state.comp_plan_scores = plan_scores
    st.session_state.comp_subgoal_data = subgoal_data
    st.session_state.comp_subgoal_quality = subgoal_quality
    st.session_state.comp_tool_efficacy = tool_efficacy
    st.session_state.comp_latency_secs = latency_secs
...
# ── Show Previous Run (persisted across tab switches) ─────────────────────────
...
elif "comp_agent_result" in st.session_state and "comp_all_scores" in st.session_state:
    prev_idx = st.session_state.get("comp_selected_idx", 0)
    ...
    display_results(
        st.session_state.comp_agent_result,
        prev_item,
        st.session_state.comp_all_scores,
        st.session_state.comp_judge_scores,
        st.session_state.get("comp_plan_scores", {"plan_quality": 0, "plan_adherence": 0, "step_efficiency": 0, "reasoning": ""}),
        st.session_state.get("comp_subgoal_data", {"subgoals": [], "coverage_pct": 0}),
        st.session_state.get("comp_subgoal_quality", {"completeness": 0, "logical_order": 0, "reasoning": ""}),
        st.session_state.get("comp_tool_efficacy", []),
        st.session_state.comp_trace_id,
